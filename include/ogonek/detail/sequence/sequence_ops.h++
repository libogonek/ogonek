...
namespace ogonek {
    namespace detail {
        ...
        // concept Sequence {
            ...
            //             a [concept:SmartSequence]; or
            //             a pointer to the first character in a null-terminated string.
            // nonmember unspecified forward_as_sequence(Sequence const&);
        // };

        //! {traits}
        //! *Note*: implementation backend for [traits:sequence_traits].
        template <typename S,
                  bool = is_smart_sequence<S>()>
        struct sequence_traits_impl {};

        //! {specialization:1}
        template <typename S>
        struct sequence_traits_impl<S, true> {
            //! {trait}
        ...
        struct sequence_traits_impl<std::pair<Iterator, Iterator>, false> {
        ...
        struct sequence_traits_impl<Char const*, false> {
        ...
        //! {traits}
        //! Provides a unified interface for using models of [concept:Sequence].
        //! *Requires*: `S` is a type returned from [function:forward_as_sequence] [soft].
        template <typename S>
        struct sequence_traits : sequence_traits_impl<wheels::Unqualified<S>> {};
        ...
        namespace sequence {
            ...
            template <typename S>
            using Value = typename sequence_traits<S>::value_type;
            ...
            //! *Note*: this type may not be a reference if the sequence `S` is not persistent.
            template <typename S>
            using Reference = typename sequence_traits<S>::reference;

            //! {function}
            //! *Requires*: `S` is a type returned from [function:forward_as_sequence] [soft].
            //! *Returns*: `true` if the sequence `s` has no elements.
            template <typename S>
            bool empty(S const& s) { return sequence_traits<S>::empty(s); }
            ...
            //! *Returns*: the first element of the sequence `s`.
            template <typename S>
            Reference<S> front(S const& s) { return sequence_traits<S>::front(s); }
            ...
            //! *Effects*: skips the first element in the sequence `s`.
            template <typename S>
            void pop_front(S& s) { return sequence_traits<S>::pop_front(s); }

            //! {function}
            //! *Requires*: `S` is a type returned from [function:forward_as_sequence] [soft].
            //! *Returns*: a sequence in the current state of `s`.
            template <typename S>
            S save(S const& s) { return sequence_traits<S>::save(s); }
        } // namespace sequence
    } // namespace detail
} // namespace ogonek
